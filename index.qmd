...
message_themes <- list(
  ...
  general = list(
    warning = c(
      "Heads up! Your document contains some of the keywords you're screening for.",
      "Keywords detected! Some terms from your list appear in this document.",
      "Keyword match found! Your document contains terms from your screening list.",
      "Screening alert: Keywords from your list were found in this document.",
      "Attention: Your document contains monitored keywords from your list."
    ),
    success = c(
      "All clear! Your document doesn't contain any keywords from your list.",
      "No matches found! None of your keywords appear in this document.",
      "Good to go! Your document is free of all the keywords you're looking for.",
      "Clean scan! No keywords from your list were detected in this document.",
      "No hits! Your document doesn't contain any terms from your screening list."
    )
  )
)
...
ui <- page_fluid(
  ...
  # App title
  h2("Keyword Screener", class="text-center mb-4"),
    ...
    column(
      ...
      tabsetPanel(
        ...
        tabPanel(
          "Keywords",
          br(),
          radioButtons("keyword_source", "Keyword Source:", 
                       choices = c("Default MAGA Keywords" = "default",
                                   "Upload Your Own CSV" = "custom"),
                       selected = "default"),
          
          # Custom keywords upload
          conditionalPanel(
            condition = "input.keyword_source == 'custom'",
            fileInput("keywords_file", "Upload Keywords CSV",
                      accept = c(".csv", ".txt")),
            tags$p("CSV should have one keyword per row in a single column.")
          ),
          
          # Display of current keywords
          h5("Current Keywords:", class = "mt-3"),
          htmlOutput("keywords_display"),
          
          # Message style selection
          radioButtons("message_style", "Feedback Style:", 
                       choices = c("MAGA Theme" = "maga",
...
server <- function(input, output, session) {
  ...
  # Reactive value for active keywords
  active_keywords <- reactiveVal(default_keywords)
  ...
  # Update active keywords based on source selection
  observe({
    if (input$keyword_source == "default") {
      active_keywords(default_keywords)
    } else if (input$keyword_source == "custom" && !is.null(input$keywords_file)) {
      # Read custom keywords from uploaded file
      tryCatch({
        # Try reading as CSV first
        keywords_df <- read.csv(input$keywords_file$datapath, stringsAsFactors = FALSE, header = FALSE)
        custom_keywords <- as.character(keywords_df[[1]])
        
        # If no keywords found or error, try reading as plain text
        if (length(custom_keywords) == 0) {
          custom_keywords <- readLines(input$keywords_file$datapath)
        }
        
        # Clean keywords (remove empty strings)
        custom_keywords <- custom_keywords[custom_keywords != ""]
        
        # Update active keywords
        if (length(custom_keywords) > 0) {
          active_keywords(custom_keywords)
        } else {
          # If no valid keywords, revert to default
          showNotification("No valid keywords found in file. Using default keywords.", type = "warning")
          active_keywords(default_keywords)
        }
      }, error = function(e) {
        showNotification(paste("Error reading keywords file:", e$message), type = "error")
        # On error, revert to default
        active_keywords(default_keywords)
      })
    }
  })
  ...
  output$keywords_display <- renderUI({
    keywords <- active_keywords()
    
    if (length(keywords) > 20) {
      displayed_keywords <- keywords[1:20]
      keyword_text <- paste(displayed_keywords, collapse = ", ")
      keyword_text <- paste0(keyword_text, ", ... and ", length(keywords) - 20, " more")
    } else {
      keyword_text <- paste(keywords, collapse = ", ")
    }
    ...
    tags$div(
      ...
      tags$div(
        ...
        tags$small(paste(length(keywords), "keywords total"))
      )
    )
  })
  ...
  observeEvent(input$analyze_btn, {
    ...
    withProgress(message = 'Analyzing document...', value = 0.2, {
      ...
      keyword_results <- find_keywords(text, active_keywords())
  ...
  output$document_info <- renderText({
    ...
    paste0(
      ...
      "Keyword Source: ", 
      ifelse(results()$keyword_source == "default", "Default MAGA Keywords", "Custom Keywords")
    )
  })
  
  # Result messages based on theme selected
  output$result_message <- renderUI({
    if (is.null(results())) return(NULL)
    
    # Get the appropriate message set based on theme
    theme <- results()$message_style
    ...
    if (length(results()$keywords_found) > 0) {
      ...
      div(
        class = "result-message result-warning",
        sample(message_themes[[theme]]$warning, 1),
        tags$br(),
        tags$span(
      ...
      div(
        class = "result-message result-success",
        sample(message_themes[[theme]]$success, 1)
      )
    }
  })
